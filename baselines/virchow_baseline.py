...
def main():
    usage = "usage: python baselines/virchow_baseline.py [config.yaml] [checkpoint_path=/path] [output_dir=/path]"
    config_path = REPO_DIR / "configs" / "main.yaml"
    checkpoint_path = Path("/data/Virchow")
    output_dir = Path(os.path.expandvars("/data/$USER/nanopath/baselines/virchow"))
    ...
    cfg["project"]["name"] = "baseline-virchow"
    cfg["project"]["family"] = "baseline"
    cfg["project"]["recipe_id"] = "virchow-vith14-untouched"
    cfg["project"]["output_dir"] = str(output_dir)
    cfg["data"]["mean"] = [0.485, 0.456, 0.406]
    cfg["data"]["std"] = [0.229, 0.224, 0.225]
    cfg["model"]["type"] = "virchow"
    cfg["probe"]["enabled"] = True
    cfg["probe"]["model_weights"] = "ema"
    cfg["probe"]["count"] = 1
    cfg["probe"]["model_loader"] = "baselines.virchow_baseline:load_probe_model"
    ...
    request = {
        ...
        "job_id": f"{os.environ.get('SLURM_JOB_ID', 'local')}-virchow",
        "config": cfg,
    ...
    summary = {
        ...
        "checkpoint_path": str(checkpoint_path),
        "backbone_activated_params": 632_000_000,
        "steps_completed": 0,
        "train_flops": 0,
