...
# rejoins the sequence. Wrapping a token-wise layer (LayerNorm, qkv Linear) this way means Block and
# Attention need no forward changes, and attention still sees one unmasked sequence so SDPA keeps its
# Flash-2 path. The two copies are loaded from the same pretrained tensor, so they only diverge through
# training. Splitting a Linear costs no extra FLOPs: every token still passes through exactly one.
